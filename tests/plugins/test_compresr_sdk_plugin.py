...
def test_prompt_requires_env_for_key_non_tty_hints_instead_of_prompting(monkeypatch):
    ...
    Exercises the real ``_prompt_plugin_env_vars`` — the non-TTY gate lives
    there so the direct install path is covered too."""
    ...
    ), patch("builtins.input", side_effect=AssertionError("prompted on non-TTY")):
