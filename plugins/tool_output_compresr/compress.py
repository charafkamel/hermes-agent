...
def compress_tool_output(
    ...
    if out_tok >= base_tok:
        ...
        # concurrent/prior compression of identical content may already have
        # handed this path to the model. The pruner reclaims it if unreferenced.
        # Benign "no net win" (not a failure) — leave info["error"] unset so the
        # caller doesn't back off or count it as an error.
