...
def _prompt_requires_env_for_key(key: str, console) -> None:
    ...
    try:
        ...
        if not sys.stdin.isatty():
            ...
            names = [n for n in (s if isinstance(s, str) else (s.get("name") or "") for s in req) if n]
            missing = [n for n in names if not get_env_value(n)]
            if missing:
                console.print(f"[dim]Set {', '.join(missing)} in {display_hermes_home()}/.env to activate.[/dim]")
            return
